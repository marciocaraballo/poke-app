...
const PokeDetailsPanel = (props: PokeDetailsProps) => {
    ...
    useEffect(() => {
        ...
        async function fetchPokemonDetails(pokemonUrl: string) {
            try {
                const pokemonDetailsResponse = await getPokemonDetails(pokemonUrl);
    ...
    if (!selectedPokemonUrl) {
        return (
            <aside className="poke-details">Please click on a PokeCard from the grid</aside>
        );
    }
        ...
        return (
            <aside className="poke-details">
                <div className="poke-details__panel">
                    ...
                    <div>
                        {
                            (pokemonDetails.frontImageUrl === null || pokemonDetails.backImageUrl === null) ?
                                <p>No images available</p> :
                                <>
                                    <img src={pokemonDetails.frontImageUrl} alt={pokemonDetails.name}/>
                                    <img src={pokemonDetails.backImageUrl} alt={pokemonDetails.name}/>
                                </>
                        }
                    </div>
                    <div>
                        ...
                        <p><strong>Abilities: </strong> {pokemonDetails.abilities.map(pokemonAbility => pokemonAbility.ability.name).join(', ')}</p>
                    </div>
                </div>
            </aside>
        );
