...
const filterByPokemonlist = (nameOrIdFilter: string, pokemonList: ReadonlyArray<Pokemon>): ReadonlyArray<Pokemon> => {
    ...
    if (Number.isNaN(parsedInput)) {
        return pokemonList.filter(pokemon => pokemon.name.startsWith(nameOrIdFilter));
    } else {
        /** 
         ...
         */
        return pokemonList.filter(pokemon => pokemon.url === `https://pokeapi.co/api/v2/pokemon/${parsedInput}/`);
    }
}
