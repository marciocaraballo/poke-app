...
const listPokemons = async () => await fetchUtil<PokemonListResponse>('GET', buildUrlWithHostname("/pokemon?limit=3000"));
...
const getPokemonsByAbility = async (abilityName: string) =>  {
    const ability = await fetchUtil<any>('GET', buildUrlWithHostname(`/ability/${abilityName}`));
...
const getPokemonsByAbilities = async (abilities: Array<string>) => {
    const promises = abilities.map(abilityName => getPokemonsByAbility(abilityName));

    const results = await Promise.all(promises);

    const concatResults = results.reduce((acc, result) => acc.concat(result), []);
